...
export const STOPS = [
  ...
  { id:'lion-bar-roslagsgatan', name:'Lion Bar Vasastan', address:'Roslagsgatan 20', area:'Vasastan', type:'Pub', food:true, lat:59.34235, lng:18.06695 },
  { id:'mondo-tapas', name:'Mondo Tapas Bar', address:'Odengatan 47', area:'Odenplan', type:'Bar / tapas', food:true, lat:59.34325, lng:18.05775 },
  { id:'bishops-sankt-eriksgatan', name:'The Bishops Arms Sankt Eriksgatan', address:'Sankt Eriksgatan 115', area:'Vasastan', type:'Gastropub', food:true, lat:59.34605, lng:18.04035 },
  { id:'cliff-barnes', name:'Cliff Barnes', address:'Norrtullsgatan 45', area:'Vasastan', type:'Bar / restaurang', food:true, lat:59.34705, lng:18.05065 },
  { id:'babette', name:'Babette', address:'Roslagsgatan 6', area:'Vasastan', type:'Vinbar / restaurang', food:true, lat:59.34035, lng:18.06775 },
  { id:'alegria', name:'Alegria Tapas Bar', address:'Sankt Eriksplan 2A', area:'S:t Eriksplan', type:'Tapasbar', food:true, lat:59.33918, lng:18.03905 },
];
...
export function nextChoices(current, visitedIds, { foodMode = false } = {}) {
  ...
  if (foodMode) {
    ...
    if (withFood.length >= 3) pool = withFood;
  }

  // Primary rule: offer three options within roughly ten minutes' walk.
  let near = pool.filter((s) => s.mins <= 10);

  // At the edge of the venue pool, use the nearest candidates instead of dead-ending.
  if (near.length < 3) {
    near = [...pool].sort((a, b) => a.mins - b.mins).slice(0, 10);
  }

  // Prefer variation: avoid returning three venues of the same type when possible.
  const shuffled = shuffle(near);
  const choices = [];

  for (const venue of shuffled) {
    if (!choices.length || choices.some((c) => c.type !== venue.type)) {
      choices.push(venue);
    }
    if (choices.length === 3) break;
  }

  // Fill any remaining slot regardless of type.
  for (const venue of shuffled) {
    if (!choices.some((c) => c.id === venue.id)) choices.push(venue);
    if (choices.length === 3) break;
  }
