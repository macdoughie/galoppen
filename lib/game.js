...
export async function findFinishedSession(rideName) {
  const wanted = normalizeRideName(rideName);
  if (!wanted) throw new Error('Skriv Galoppens namn.');

  const snap = await getDocs(collection(db, 'galoppenSessions'));
  const matches = snap.docs
    .map((d) => ({ ...d.data(), rideId: d.id }))
    .filter((s) =>
      (s.status === 'finished' || s.phase === 'finished') &&
      normalizeRideName(s.rideName || '') === wanted
    )
    .sort((a, b) => {
      const at = a.savedAt?.seconds || a.createdAt?.seconds || 0;
      const bt = b.savedAt?.seconds || b.createdAt?.seconds || 0;
      return bt - at;
    });

  if (!matches.length) throw new Error('Jag hittar ingen avslutad Galopp med det namnet.');
  return matches[0].rideId;
}
...
export function watchActiveSessions(callback) {
  return onSnapshot(collection(db, 'galoppenSessions'), async (snap) => {
    ...
    const rows = await Promise.all(sessions.map(async (s) => {
      ...
      const rows = ps.docs.map((d) => ({ ...d.data(), uid: d.id }));
      const replacedUids = new Set(rows.map((p) => p.replacesUid).filter(Boolean));
      const playerCount = rows.filter((p) => p.active !== false && !replacedUids.has(p.uid)).length;
...
export async function joinSession({ rideId, user, nickname }) {
  ...
  if (!cleanNickname) throw new Error('Skriv ditt smeknamn.');
  ...
  const allPlayers = ps.docs.map((d) => ({ ...d.data(), uid: d.id }));

  // Ignore rider records that have been superseded by a reconnect.
  const replacedUids = new Set(
    allPlayers.map((p) => p.replacesUid).filter(Boolean)
  );
  const activePlayers = allPlayers.filter(
    (p) => p.active !== false && !replacedUids.has(p.uid)
  );

  const normalizeNick = (value) => (value || '').trim().toLocaleLowerCase('sv-SE');
  const returning = activePlayers.find(
    (p) => normalizeNick(p.nickname) === normalizeNick(cleanNickname)
  );

  // A returning rider is allowed back even if the Galopp is full.
  if (returning) {
    await setDoc(playerRef(rideId, user.uid), {
      ...returning,
      uid: user.uid,
      nickname: returning.nickname || cleanNickname,
      joinedAt: serverTimestamp(),
      rejoinedAt: serverTimestamp(),
      replacesUid: returning.uid,
      active: true,
    });

    const sessionUpdates = {};
    if (session.currentRiderUid === returning.uid) sessionUpdates.currentRiderUid = user.uid;
    if (session.currentPayerUid === returning.uid) sessionUpdates.currentPayerUid = user.uid;
    if (session.hostUid === returning.uid) sessionUpdates.hostUid = user.uid;
    if (session.ownerUid === returning.uid) sessionUpdates.ownerUid = user.uid;

    if (Object.keys(sessionUpdates).length) {
      await updateDoc(sessionRef(rideId), sessionUpdates);
    }

    return rideId;
  }

  if (session.capacity && activePlayers.length >= session.capacity) {
    throw new Error('Galoppen är full. Är du redan med? Skriv samma smeknamn som tidigare för att återansluta.');
  }
...
export function watchPlayers(code, callback) {
  return onSnapshot(collection(db, 'galoppenSessions', code, 'players'), (snap) => {
    ...
    const players = rows
      .filter((p) => !replacedUids.has(p.uid))
      .sort((a, b) => {
